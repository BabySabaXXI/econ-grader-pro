 ...
 * Routes messages between content script, grading panel (iframe), and Vercel API backend.
 * No sidePanel API — the panel is an iframe injected by the content script.
 *
...
/**
 * Forward a message to the Google Doc tab's content script.
 * Searches for a Google Doc tab since the panel is an iframe inside it.
 */
function forwardToDocTab(message, sendResponse) {
  chrome.tabs.query({ url: "https://docs.google.com/document/*" }, (tabs) => {
    if (tabs && tabs.length > 0) {
      // Try the active one first, fall back to the first found
      const active = tabs.find(t => t.active) || tabs[0];
      chrome.tabs.sendMessage(active.id, message, (response) => {
 ...
 * Finds the Google Doc tab explicitly.
 */
function handleExtractDocText(sendResponse) {
  chrome.tabs.query({ url: "https://docs.google.com/document/*" }, async (tabs) => {
    let tab = null;
    if (tabs && tabs.length > 0) {
...
async function extractViaMainWorldFetch(tabId, docId) {
  ...
  if (result?.text) return result.text.replace(/^\uFEFF/, "").replace(/\r\n/g, "\n").trim();
  return "";
}

async function extractViaBackgroundFetch(docId) {
  const exportUrl = `https://docs.google.com/document/d/${docId}/export?format=txt`;
  const response = await fetch(exportUrl, { credentials: "include", redirect: "follow" });
    ...
    func: () => {
      try {
        ...
        for (const script of scripts) {
          ...
          if (content.includes("DOCS_modelChunk") || content.includes("DOCS_modelData") || content.includes("kix-model")) {
            modelData += content + "\n";
          }
        }
        if (!modelData) return "";
        ...
        if (strings.length > 0) { const text = strings.join(""); if (text.trim().length > 10) return text.trim(); }
        const arrayPattern = /\["([^"]{5,}(?:","[^"]*)*?)"\]/g;
        const arrayStrings = [];
        while ((match = arrayPattern.exec(modelData)) !== null) {
          const parts = match[0].slice(1, -1).split(",")
            .map((s) => { try { return JSON.parse(s); } catch { return ""; } }).filter(Boolean);
          arrayStrings.push(...parts);
        }
        if (arrayStrings.length > 0) { const text = arrayStrings.join(" "); if (text.trim().length > 10) return text.trim(); }
        return "";
      } catch (e) { return ""; }
    },
  });
...
async function extractViaAccessibility(tabId) {
  const results = await chrome.scripting.executeScript({
    ...
    func: () => {
      try {
        const textParts = [];
        const annotatedElements = document.querySelectorAll("[aria-label]:not([role='button']):not([role='toolbar']):not([role='menubar'])");
        for (const el of annotatedElements) {
          const label = el.getAttribute("aria-label") || "";
          if (label.length > 20 && !label.startsWith("Menu") && !label.startsWith("Toolbar")) textParts.push(label);
        }
        if (textParts.join("").length > 50) return textParts.join("\n").trim();
        const textboxes = document.querySelectorAll('[role="textbox"]');
        for (const tb of textboxes) { const text = tb.innerText || tb.textContent || ""; if (text.trim().length > 10) return text.trim(); }
        const iframe = document.querySelector(".docs-texteventtarget-iframe");
        if (iframe) { try { const iframeDoc = iframe.contentDocument || iframe.contentWindow?.document; if (iframeDoc) { const text = iframeDoc.body?.innerText || iframeDoc.body?.textContent || ""; if (text.trim().length > 10) return text.trim(); } } catch (e) {} }
        const srElements = document.querySelectorAll(".docs-a11y-ariascreenreader-container, [role='document']");
        for (const el of srElements) { const text = el.innerText || el.textContent || ""; if (text.trim().length > 10) return text.trim(); }
        const editorArea = document.querySelector(".kix-appview-editor");
        if (editorArea) {
          const allText = [];
          const walk = (node) => {
            if (node.nodeType === Node.TEXT_NODE) { const t = node.textContent.trim(); if (t.length > 0) allText.push(t); }
            else if (node.nodeType === Node.ELEMENT_NODE) { const style = window.getComputedStyle(node); if (style.display === "none" || style.visibility === "hidden") return; for (const child of node.childNodes) walk(child); }
          };
          walk(editorArea);
